...
class TaxiEnv(discrete.DiscreteEnv):
    ...
    def __init__(self):
        ...
        isd = np.zeros(nS)
        nA = 6
        change_taxi_location = 0
        change_passanger_location = 0
        change_dest_location = 0
                ...
                for passidx in range(5):
                    for destidx in range(4):
                        ...
                        for a in range(nA):
                            ...
                            if a==0:
                                newrow = min(row+1, maxR) #down
                            elif a==1:
                                newrow = max(row-1, 0)  #up
                            if a==2 and self.desc[1+row,2*col+2]==b":":
                                newcol = min(col+1, maxC) #right
                            elif a==3 and self.desc[1+row,2*col]==b":":
                                newcol = max(col-1, 0) #left
                            elif a==4: # pickup
                                if (passidx < 4 and taxiloc == locs[passidx]):
                                    newpassidx = 4
                                    change_passanger_location = change_passanger_location+1
                                else:
                                    reward = -2
                            elif a==5: # dropoff
                            ...
                            self.g.add_edge(5*row+col,5*newrow+newcol)
                            if state != newstate:
                                change_taxi_location = change_taxi_location+1
                            P[state][a].append((1.0, newstate, reward, done))
        isd /= isd.sum()
        print `change_taxi_location` + " " +`change_passanger_location` +" "+ `change_dest_location`
        nx.draw(self.g, with_labels = True)
        plt.show()
        [len(c) for c in sorted(nx.strongly_connected_components(self.g),key=len, reverse=True)]
